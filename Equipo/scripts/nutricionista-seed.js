/**
 * Crea la base de datos nutricionista y carga los menus semanales.
 *
 * Ejecutar: node scripts/nutricionista-seed.js
 */
...
async function main() {
  ...
  console.log('--- Creando base de datos nutricionista ---');
  await conn.query('CREATE DATABASE IF NOT EXISTS nutricionista CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci');
  await conn.query('USE nutricionista');
  ...
  console.log('--- Tabla nutri_menus OK ---');
  ...
  const stmt = 'INSERT INTO nutri_menus (dia, comida, descripcion) VALUES (?, ?, ?)';
  for (const row of menus) {
    await conn.query(stmt, row);
  }

  console.log(`--- ${menus.length} registros insertados ---`);

  // Verificar
  const [count] = await conn.query('SELECT COUNT(*) as total FROM nutri_menus');
  const [dias] = await conn.query('SELECT dia, COUNT(*) as comidas FROM nutri_menus GROUP BY dia');
  console.log('Total registros:', count[0].total);
  console.log('Por dia:');
  dias.forEach(d => console.log(`  ${d.dia}: ${d.comidas} comidas`));
